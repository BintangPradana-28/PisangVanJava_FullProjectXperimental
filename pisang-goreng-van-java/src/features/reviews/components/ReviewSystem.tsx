  ...
  return (
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map(star => (
        ...
        >
          <span className={star <= (hovered || value) ? 'text-amber-400' : 'text-zinc-200 dark:text-zinc-700'}>
            ★
          </span>
        </button>
      ))}
...
export default function ReviewSystem({ initialReviews, initialAggregates, currentFilter }: ReviewSystemProps) {
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!session) { toast.error('Login dahulu untuk memberikan ulasan'); return }
    if (rating === 0) { toast.error('Pilih bintang rating terlebih dahulu'); return }
    if (!variantId) { toast.error('Pilih varian menu yang ingin diulas'); return }
    ...
    } catch { toast.error('Koneksi bermasalah. Coba lagi.') }
  ...
  return (
    <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6">
      ...
      <div className="max-w-[1200px] mx-auto mb-8 text-center">
        <div className="text-secondary text-xs font-bold tracking-[0.2em] uppercase mb-3">{t('review_subtitle')}</div>
        ...
        <p className="mt-4 text-zinc-600 dark:text-zinc-400 max-w-2xl mx-auto">{t('review_desc')}</p>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-8">
          ...
          <div className="flex-1 w-full space-y-2">
            {[5, 4, 3, 2, 1].map(star => {
              ...
              return (
                <button key={star} onClick={() => handleFilterClick(String(star))}
      ...
      <div className="mb-6">
        {session ? (
            ...
            {!showForm ? (
              ...
              >
                <div className="flex justify-between items-center">
                  <h3 className="font-serif text-xl font-bold text-zinc-900 dark:text-zinc-100">✍️ Tulis Ulasan</h3>
                ...
                <div>
                  ...
                  {rating > 0 && (
                    <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      className="text-xs text-amber-600 font-semibold mt-1">{STAR_LABELS[rating]}</motion.p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-400 mb-2">Masukkan ID varian (dari riwayat pesanan Anda). Sistem akan memverifikasi pembelian Anda.</p>
                  <input
                    type="text" value={variantId}
                  ...
                  <p className="text-xs text-zinc-400 mt-1">
                    💡 Tip: Buka{' '}
                    <Link href="/track-order" className="text-amber-500 hover:underline font-medium">Riwayat Pesanan</Link>
                ...
                <div className="flex items-start gap-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50 rounded-2xl p-4">
                  ...
                  <p className="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
                    <strong>Verified Buyer Check:</strong> Sistem kami akan otomatis memverifikasi apakah Anda pernah membeli varian ini. Jika ya, ulasan Anda akan mendapat badge <strong>✅ Pembeli Terverifikasi</strong>.
                  </p>
                </div>
          ...
          <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 text-center">
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-3">Ingin berbagi pengalaman Anda?</p>
            <Link href="/member-login?callbackUrl=/ulasan"
      ...
      {/* ── Filter Chips ── */}
      <div className="flex overflow-x-auto pb-4 mb-6 gap-3 border-b border-zinc-200 dark:border-zinc-800" style={{ scrollbarWidth: 'none' }}>
        {FILTERS.map(f => {
          const isSelected = optimisticFilter === f
          const label =
            f === 'Semua'             ? t('review_filter_all') :
            f === 'Dengan Komentar'   ? t('review_filter_comment') :
      ...
      <div className={`space-y-6 transition-opacity duration-300 ${isPending ? 'opacity-50' : 'opacity-100'}`}>
        {initialReviews.length === 0 ? (
          <div className="text-center py-16 bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800">
            ...
            <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 mb-2">{t('review_empty_title')}</h3>
            ...
            {initialReviews.map(review => (
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-bold text-zinc-900 dark:text-zinc-100 text-sm">{review.userName}</span>
                ...
                <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30 rounded-xl text-xs font-semibold text-amber-700 dark:text-amber-400">
                  ...
                  <span className="text-[#D4802A] uppercase tracking-wide">{review.variantName}</span>
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
