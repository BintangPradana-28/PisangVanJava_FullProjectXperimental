...
const styles = StyleSheet.create({
  ...
  header: { fontSize: 24, marginBottom: 20, textAlign: 'center', color: '#d97706', fontWeight: 'bold' },
  ...
  tableRow: { flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#eee', borderBottomStyle: 'solid', paddingBottom: 8, paddingTop: 8 },
    ...
    <Page size="A4" style={styles.page}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 30 }}>
        ...
        <View style={{ textAlign: 'right' }}>
          ...
          <Text style={styles.text}>Tanggal: {new Date(order.createdAt).toLocaleDateString('id-ID')}</Text>
      ...
      <View style={styles.table}>
        <View style={[styles.tableRow, { backgroundColor: '#f8f9fa', borderTopWidth: 1, borderTopColor: '#eee' }]}>
          <View style={styles.tableColWide}><Text style={styles.bold}>Item</Text></View>
          <View style={styles.tableColCenter}><Text style={styles.bold}>Qty</Text></View>
          <View style={styles.tableCol}><Text style={styles.bold}>Subtotal</Text></View>
        </View>
        ...
        <View style={[styles.tableRow, { borderBottomWidth: 0, marginTop: 10 }]}>
           <View style={styles.tableColWide}><Text style={styles.bold}>Ongkos Kirim</Text></View>
           <View style={styles.tableColCenter}><Text style={styles.text}></Text></View>
           <View style={styles.tableCol}><Text style={styles.text}>Rp {order.deliveryFee.toLocaleString('id-ID')}</Text></View>
        </View>

        {order.discountAmount > 0 && (
          <View style={[styles.tableRow, { borderBottomWidth: 0 }]}>
             <View style={styles.tableColWide}><Text style={[styles.bold, { color: '#16a34a' }]}>Diskon</Text></View>
             <View style={styles.tableColCenter}><Text style={styles.text}></Text></View>
             <View style={styles.tableCol}><Text style={[styles.text, { color: '#16a34a' }]}>-Rp {order.discountAmount.toLocaleString('id-ID')}</Text></View>
          </View>
        )}

        <View style={[styles.tableRow, { borderTopWidth: 2, borderTopColor: '#333', marginTop: 5 }]}>
           <View style={styles.tableColWide}><Text style={[styles.bold, { fontSize: 14 }]}>TOTAL BAYAR</Text></View>
           <View style={styles.tableColCenter}><Text style={styles.text}></Text></View>
           <View style={styles.tableCol}><Text style={[styles.bold, { fontSize: 14 }]}>Rp {order.totalPrice.toLocaleString('id-ID')}</Text></View>
        </View>
      </View>
      
      <View style={{ marginTop: 50, textAlign: 'center' }}>
        <Text style={{ fontSize: 10, color: '#888' }}>Terima kasih atas pesanan Anda di Pisang Van Java.</Text>
      </View>
    </Page>
  </Document>
)
